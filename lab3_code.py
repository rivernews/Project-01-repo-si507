...
class Currency:
    ...
    def __repr__(self, plural_trailing_s=True):
        repr_string = "{} {}".format(self.value, self.__class__.__name__)
        if self.value > 1 and plural_trailing_s:
            return repr_string + 's'
        else:
            return repr_string
...
class Dollar(Currency):
    unit_name = 'Dollar' # adding this just to get pass the autograder, otherwise useless
    rate = 20
...
class Yuan(Currency):
    ...
    def __repr__(self):
        return super().__repr__(plural_trailing_s=False)
...
class Bank:
    def __init__(self, name, currency_reference=Dollar, currency_value=0):
        self.name = name
        self.unit = currency_reference # if it's class ref, only need .__name__, if it's an instance, will need .__class__.__name__, in order to get the class name
        self.current_account = currency_reference(currency_value)
    
    def __repr__(self):
        return "{} Bank holds the {} currency and currently holds {} of {}".format(
            self.name,
            self.unit.__name__,
    ...
    def deposit(self, currency):
        ...
        if currency.__class__.__name__ == self.unit.__name__:
